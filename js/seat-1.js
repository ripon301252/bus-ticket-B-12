// // seat button delegation technique
// document.getElementById('seat-container').addEventListener('click', function(e)  {
//     if(e.target.className.includes('seat-title')){
//         const seatBtn = e.target;

//         const allSeat = seatBtn.parentNode.children[1].innerText
//         console.log(allSeat)
//     }
//  })


// seat button traverse technique
const SeatBtns1 = document.getElementsByClassName('seat-title-1')
for (let seatBtn1 of SeatBtns1) {
  seatBtn1.addEventListener('click', function () {

    // add classList
    seatBtn1.classList.add('seat-color')

    // seat-number
    const seat1 = seatBtn1.parentNode.children[1].innerText;

    // per seat price
    const seatRent = seatBtn1.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.children[2].children[2].children[0].children[2].children[1].children[0].innerText;
    const totalPrice = document.getElementById('total-price').innerText
    const newTotalPrice = Number(seatRent) + Number(totalPrice)

    // total seat price
    const allSeatPrice = document.getElementById('total-price').innerText = newTotalPrice

    // grand-total-price
    document.getElementById('grand-total-price').innerText = allSeatPrice;

    // seat-left
    const seatLess = seatBtn1.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.children[2].children[2].children[0].children[0].children[0].children[1].children[0].children[1].children[0].children[0].innerText;
    const seatLeft = Number(seatLess) - 1
    document.getElementById('seat-left').innerText = seatLeft

    // Available-seat
    const availableSeat = seatBtn1.parentNode.parentNode.parentNode.parentNode.children[1].children[1].children[0].children[0].children[0].children[0].children[0].innerText;
    const haveSeat = Number(availableSeat) + 1;
    document.getElementById('seat-count').innerText = haveSeat;



    const seatContent1 = document.getElementById('seating')
    const newSeatContent1 = document.createElement('div');
    // newSeatContent1.classList.add('seat-color')
    newSeatContent1.innerHTML = `
                <div class=" flex justify-between items-center">
                  <h1 id="seat-name-1">${seat1}</h1>
                  <h1 id="catagory-1" class="pl-20">Economy</h1>
                  <h1 id="seat-price-1" class="pl-20">${seatRent}</h1>
                </div>
        `
    seatContent1.appendChild(newSeatContent1)
  })
}




